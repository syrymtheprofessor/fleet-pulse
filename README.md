...
Реализовать **3 пункта**. Ориентир по времени: 2-3 часа.
...
### 3. MockMvc-тест на endpoint

Напиши один тест через `@WebMvcTest` + `@MockBean`. Проверь, что endpoint возвращает корректный JSON для существующей машины с поездкой и 404, если поездок нет.
